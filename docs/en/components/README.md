...
For a short implementation cookbook with donor-module lessons and anti-custom
rules, start with [EvoUI Implementation Lessons](../guide/implementation-lessons.md).
...
| Assets | `evo::partials.assets`, `LivewireAssets` | evo-ui | none |
...
Use [Action Buttons](action-buttons.md) as the canonical guide for form save
buttons, table toolbar actions, table control actions, row actions and modal
footer actions.
...
| `tone` | string | `neutral` | `neutral`, `primary`, `success`, `info`, `warning`, `danger`. |
| `variant` | string | `soft` | `soft` or `filled`. |
...
The full action taxonomy is documented in [Action Buttons](action-buttons.md).
...
Use EvoUI inline-create markers when an add action creates a row, tab or card
without opening a modal. Consumers own the Livewire method that mutates data;
EvoUI owns scroll, focus, highlight and optional bottom create visibility.
...
After the consumer creates an item, dispatch the shared event after the DOM has
the new row:
...
- root marker: `data-evo-inline-create`;
- new item/id markers: `data-evo-inline-created` and
  `data-evo-inline-create-id`;
- primary focus marker: `data-evo-inline-focus`;
- bottom action marker: `data-evo-inline-create-bottom`;
- runtime API: `window.EvoUI.initInlineCreate(root)` and
  `window.EvoUI.focusInlineCreateItem(item)`.
...
Use [Form Component](form.md) as the canonical guide for declarative settings,
...
Use [Form Component](form.md) as the canonical guide for standalone settings,
config, model and resource forms. Use [Form And Field Catalogue](form-fields.md) for
...
Use [Table](table/README.md) as the canonical guide for provider-backed
...
For full structure, provider hooks, toolbar rules, row actions, double-click
modal behavior, pagination and consumer donor patterns, read
[Table](table/README.md).
...
Use [Modal And Confirmation](modal.md) as the canonical guide for custom
dialogs, table create/edit modals, read-only action modals and delete
confirmations.
...
Use [Dashboard Cards](dashboard-cards.md) as the canonical guide for module
dashboard widgets that sit above tables or other operational surfaces. The
dashboard primitive owns card wrapping, span widths, responsive fallback and the
standard divider spacing before the following content.
...
[Editor Media Adapter Contract](../contracts/editor-media-adapter.md).
...
The full boundary lives in [Embedded Resource Contract](../contracts/embedded-resource.md).
...
upper module tab UX. The full boundary lives in
[dDocs Tree And Viewer Primitive Notes](../guide/ddocs-tree-viewer-notes.md).
